            ...
            // Display user info
                ...
                if ($_SESSION['selectedLocation'] == 0 || empty($_SESSION['selectedLocation'])) {
                    if ($_SESSION['foodSearch']) {
                        ...
                        $statement = $db->prepare("SELECT f.id, f.food_name, f.location_id, f.details, f.quantity, l.location_name FROM foods f INNER JOIN locations l ON f.location_id = l.id WHERE lower(f.food_name) LIKE '' AND f.added_by = :id ORDER BY f.food_name");
                        $statement->execute(array(':pattern' => '%' . $_SESSION['foodSearch'] . '%'));
