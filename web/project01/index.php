            ...
            // Display user info
                ...
                if ($_SESSION['selectedLocation'] == 0 || empty($_SESSION['selectedLocation'])) {
                    if ($_SESSION['foodSearch']) {
                        $statement = $db->prepare("SELECT f.id, f.food_name, f.location_id, f.quantity, l.location_name FROM foods f INNER JOIN locations l ON f.location_id = l.id WHERE lower(f.food_name) LIKE :pattern AND f.added_by = :id ORDER BY f.food_name");
                        $statement->execute(array(':id' => $_SESSION['userId'], ':pattern' => '%' . $_SESSION['foodSearch'] . '%'));
                    } else {
                        $statement = $db->prepare('SELECT f.id, f.food_name, f.location_id, f.quantity, l.location_name FROM foods f INNER JOIN locations l ON f.location_id = l.id WHERE f.added_by = :id ORDER BY f.food_name');
                    ...
                    while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo '</select></td><td><input type="number" value="' . $row['quantity'] . '" name="newAmount' .$row['id'] . '" min="0"></td></tr>';
                        $counter++;
                    }
                } else {
                    if ($_SESSION['foodSearch']) {
                ...
                if($_SESSION['foodSearch']) { echo ' value="' . $_POST['fname'] . '"';}
                echo '><br>';
